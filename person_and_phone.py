...
#no_persons_count = 0
#mul_persons_count = 0
def mobile_counter(email,TestID):
    #global mob_count
    #mob_count = mob_count + 1
    connect = pymysql.connect(host="localhost",user="root",password="",database="ai_proctor")
    curr = connect.cursor()
    curr.execute("SELECT * from ONLINE where EMAIL=%s AND TEST_ID=%s", (email,TestID))
    rows = curr.fetchone()
    cur = connect.cursor()
    if rows[5] == 0:
        cur.execute("UPDATE ONLINE SET MOBILE_PHONE_DETECTION=%s WHERE EMAIL=%s AND TEST_ID=%s",(1, email, TestID))
    else:
        current_mob_counter = int(rows[5]) + 1
        cur.execute("UPDATE ONLINE SET MOBILE_PHONE_DETECTION=%s WHERE EMAIL=%s AND TEST_ID=%s",(current_mob_counter,email,TestID))
...
def no_person_counter(email,TestID):
    #global no_persons_count
    #no_persons_count = no_persons_count + 1
    connect = pymysql.connect(host="localhost",user="root",password="",database="ai_proctor")
    curr = connect.cursor()
    curr.execute("SELECT * from ONLINE where EMAIL=%s AND TEST_ID=%s", (email, TestID))
    rows = curr.fetchone()
    cur = connect.cursor()
    if rows[6] == 0:
        cur.execute("UPDATE ONLINE SET NO_PERSON_DETECTION=%s WHERE EMAIL=%s AND TEST_ID=%s",(1, email, TestID))
    else:
        current_no_persons_count = int(rows[6]) + 1
        cur.execute("UPDATE ONLINE SET NO_PERSON_DETECTION=%s WHERE EMAIL=%s AND TEST_ID=%s",(current_no_persons_count, email, TestID))

    connect.commit()
    connect.close()


def multiple_person_counter(email,TestID):
    #global mul_persons_count
    #mul_persons_count = mul_persons_count + 1
    connect = pymysql.connect(host="localhost",user="root",password="",database="ai_proctor")
    curr = connect.cursor()
    curr.execute("SELECT * from ONLINE where EMAIL=%s AND TEST_ID=%s", (email, TestID))
    rows = curr.fetchone()
    cur = connect.cursor()
    if rows[4] == 0:
        cur.execute("UPDATE ONLINE SET MULTIPLE_PERSON_DETECTION=%s WHERE EMAIL=%s AND TEST_ID=%s",(1, email, TestID))
    else:
        current_mul_persons_count = int(rows[4]) + 1
        cur.execute("UPDATE ONLINE SET MULTIPLE_PERSON_DETECTION=%s WHERE EMAIL=%s AND TEST_ID=%s",(current_mul_persons_count, email, TestID))
...
def start_multiplepersons(stop,email,TestID):
    ...
    while(True):
        ...
        for i in range(nums[0]):
            ...
            if int(classes[0][i] == 67):
                ...
                mobile_counter(email,TestID)
                
        if count == 0:
            print('No person detected')
            no_person_counter(email,TestID)
        elif count > 1: 
            print('More than one person detected')
            multiple_person_counter(email,TestID)
